...
class SyncEnumValuesOp(alembic.operations.ops.MigrateOperation):
    ...
    def sync_enum_values(
        ...
        :param list new_values:
            List of enumeration values that should exist after this migrations
            executes.
        ...
        enum values without a reverse state:
            ...
            old_values: list[str]
            new_values: list[str]
        By defaulting this to False, we avoid performing our new backwards migrations logic
        ...
        if should_reverse and affected_columns is not None:
            with get_connection(operations) as conn:
                ...
                conn.execute(
                    sqlalchemy.text(
                        f'ALTER TYPE {schema}.{name} RENAME TO {name}_old'
                    )
                )
                conn.execute(
                    sqlalchemy.text(
...
def compare_enums(autogen_context, upgrade_ops, schema_names):
    ...
    SQLAlchemy/Alembic will create them as part of the usual migrations process.
    """
    to_add = set()
    for schema in schema_names:
        ...
        declared = get_declared_enums(
            autogen_context.metadata, schema, default
        )
        for name, new_values in declared.enum_definitions.items():
            old_values = defined.enum_definitions.get(name)
            # Alembic will handle creation of the type in this migrations, so
            ...
            if name in defined.enum_definitions and new_values != old_values:
                ...
                to_add.add(
                    (schema, name, old_values, new_values, affected_columns)
                )

    for schema, name, old_values, new_values, affected_columns in sorted(
        to_add
    ):
        op = SyncEnumValuesOp(
            schema,

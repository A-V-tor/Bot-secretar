...
async def expanses_journal_kb(flag_last_note=False):
    ...
    if flag_last_note:
        keyboard.row(InlineKeyboardButton(text='✏️ последняя запись', callback_data='edit_last_expenses'))
...
async def category_expenses_last_name_kb():
    ...
    keyboard.row(InlineKeyboardButton(text='оставить', callback_data='current_last_expenses'))

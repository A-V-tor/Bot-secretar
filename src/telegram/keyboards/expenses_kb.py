...
async def root_menu_expanses_kb():
    ...
    journal = InlineKeyboardButton(
        text='журнал', callback_data='show expanses'
    )
    new_note = InlineKeyboardButton(
        text='добавить', callback_data='add expanses'
    )
...
async def category_expenses_kb():
    ...
    for field in TypeExpenses:
        keyboard.row(
            InlineKeyboardButton(
                text=f'{field.value}', callback_data=f'{field}'
            )
        )
...
async def expanses_journal_kb(flag_last_note=False):
    ...
    if flag_last_note:
        keyboard.row(
            InlineKeyboardButton(
                text='✏️ последняя запись', callback_data='edit_last_expenses'
            )
        )
...
async def category_expenses_last_name_kb():
    ...
    for field in TypeExpenses:
        keyboard.row(
            InlineKeyboardButton(
                text=f'{field.value}', callback_data=f'last_{field}'
            )
        )

    keyboard.row(
        InlineKeyboardButton(
            text='оставить', callback_data='current_last_expenses'
        )
    )

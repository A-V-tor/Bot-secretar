...
async def root_workout_menu(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        msg, reply_markup=await root_menu_workout_kb()
    )


@router.callback_query(F.data == 'add workout')
async def add_workout_in_journal(
    ...
    await callback.message.delete()
    await callback.message.answer(
        'Запиши тренировку: ', reply_markup=await cansel_kb()
    )
...
async def get_workout_for_day(
    ...
    workout_service = WorkoutTelegramService(callback)
    workouts_current_day, msg = await workout_service.get_workouts_current_day(
        day, month, year
    )
    ...
    keyboard = (
        await rotation_workout()
        if workouts_current_day
        else await root_menu_workout_kb()
    )
...
@router.callback_query(F.data.startswith('&'))
async def rotation_list_current_workouts(
    callback: types.CallbackQuery, state: FSMContext
):
    ...
    await callback.message.answer(
        msg, reply_markup=await rotation_workout(), parse_mode='HTML'
    )


@router.callback_query(F.data == 'del workout')
async def delete_current_record(
    callback: types.CallbackQuery, state: FSMContext
):
    data = await state.get_data()
    await callback.message.delete()
    ...
    if len(current_workouts) == 0:
        ...
        await callback.message.answer(
            msg, reply_markup=await root_menu_workout_kb(), parse_mode='HTML'
        )
        return

    current_workouts, msg_ = await workout_service.show_workout(
        'forward', {'workouts': current_workouts}
    )
    await state.set_data({'workouts': current_workouts})

    await callback.message.answer(
        msg + msg_, reply_markup=await rotation_workout(), parse_mode='HTML'
    )

...
from ..keyboards.workout_kb import root_menu_workout_kb, render_workout_calendar, rotation_workout
...
async def get_workout_journal(callback: types.CallbackQuery):
    ...
    workout_days, msg = await workout_service.get_workout_days_for_current_month(month, year)

    await callback.message.answer(msg, reply_markup=await render_workout_calendar(month, year, workout_days))

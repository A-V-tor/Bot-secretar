...
from src.telegram.keyboards.expenses_kb import root_menu_expanses_kb, category_expenses_kb, yes_or_no_save_expenses_kb, expanses_journal_kb, category_expenses_last_name_kb, yes_or_no_save_last_note_expenses_kb
...
async def show_expanses_for_day(callback: types.CallbackQuery):
    ...
    await callback.message.answer(f'<pre>{my_table}</pre>', reply_markup=await expanses_journal_kb(flag_last_note), parse_mode="HTML")
...
async def get_category_for_expenses(callback: types.CallbackQuery, state: FSMContext):
    ...
    rus_category = getattr(TypeExpenses, category).text_value
...
async def edit_last_note(callback: types.CallbackQuery, state: FSMContext):
    ...
    msg = f"Редактирование записи по категории <b>{rus_category}</b> - сумма: <b>{last_note.text_value}</b>\n\nОставь или измени категорию"

    await callback.message.answer(msg, reply_markup=await category_expenses_last_name_kb(), parse_mode="HTML")
...
async def edit_last_note_for_new_choice_category(callback: types.CallbackQuery, state: FSMContext):
    ...
    rus_category = getattr(TypeExpenses, category.split(".")[-1]).text_value
